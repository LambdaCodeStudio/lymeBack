...
const generarRemitoOficial = async (datosRemito) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      emisorY += 10;
      doc.fontSize(9).font("Helvetica-Bold");
      doc.text(`Condición frente al IVA: ${emisor.condicionIVA}`, 40, emisorY, {
        width: bloqueWidth - 20,
      });

      emisorY += 10;
      doc.fontSize(9).font("Helvetica-Bold");
      doc.text(`Domicilio Fiscal: ${emisor.domicilioFiscal}`, 40, emisorY, {
        width: bloqueWidth - 20,
      });
      ...
      emisorY += 10;
      doc.text(`Email: ${emisor.email}`, 40, emisorY, {
        width: bloqueWidth - 20,
      });
      ...
      const clienteNombre =
        datosRemito.receptor.razonSocial ||
        datosRemito.receptor.nombreCliente ||
        "Sin especificar";
      doc.text(clienteNombre, receptorX + 45, receptorY, {
        width: bloqueWidth - 65,
      });
      ...
      doc.fontSize(8).font("Helvetica");
      doc.text(
        datosRemito.receptor.cuitDni || "Sin especificar",
        receptorX + 35,
        receptorY
      );
      ...
      doc.fontSize(8).font("Helvetica");
      doc.text(
        datosRemito.receptor.domicilioEntrega || "Sin especificar",
        receptorX + 55,
        receptorY,
        { width: bloqueWidth - 75 }
      );
      ...
      if (datosRemito.receptor.email) {
        ...
        doc.fontSize(8).font("Helvetica");
        doc.text(datosRemito.receptor.email, receptorX + 55, receptorY, {
          width: bloqueWidth - 65,
        });
        receptorY += 10;
      }
      ...
      if (datosRemito.receptor.nombreSubServicio) {
        ...
        doc.fontSize(8).font("Helvetica");
        doc.text(
          datosRemito.receptor.nombreSubServicio,
          receptorX + 65,
          receptorY,
          { width: bloqueWidth - 85 }
        );
        receptorY += 10;
      }
      ...
      if (datosRemito.receptor.nombreSubUbicacion) {
        ...
        doc.fontSize(8).font("Helvetica");
        doc.text(
          datosRemito.receptor.nombreSubUbicacion,
          receptorX + 65,
          receptorY,
          { width: bloqueWidth - 75 }
        );
      ...
      const contactos = [];
      if (datosRemito.receptor.telefono)
        contactos.push(`Tel: ${datosRemito.receptor.telefono}`);
      if (datosRemito.receptor.email)
        contactos.push(`Email: ${datosRemito.receptor.email}`);
      const contactoDestino =
        datosRemito.receptor.telefEmail || "Sin especificar";
      doc.text(contactoDestino, receptorX + 100, receptorY, {
        width: bloqueWidth - 130,
      });
      ...
      const supervisorNombre =
        datosRemito.supervisor?.nombre ||
        datosRemito.supervisor?.usuario ||
        "A definir";
      ...
      const transportistaNombre =
        datosRemito.transporte?.chofer?.nombre || "A definir";
      ...
      if (datosRemito.transporte?.vehiculo) {
        const vehiculoCompleto = `${
          datosRemito.transporte.vehiculo.marca || ""
        } ${datosRemito.transporte.vehiculo.modelo || ""} - Patente: ${
          datosRemito.transporte.vehiculo.patente || ""
        }`.trim();
      ...
      doc.fontSize(8).font("Helvetica");
      const origen =
        datosRemito.transporte?.domicilioOrigen || "Sin especificar";
      ...
      doc.fontSize(8).font("Helvetica");
      const destino =
        datosRemito.transporte?.domicilioDestino || "Sin especificar";
      ...
      if (datosRemito.detalle) {
        ...
        const detalleHeight = doc.heightOfString(
          datosRemito.detalle,
          detalleOptions
        );
      ...
      if (
        datosRemito.notas &&
        Array.isArray(datosRemito.notas) &&
        datosRemito.notas.length > 0
      ) {
...
const generarRemito = async (req, res) => {
  try {
    const datosRemito = req.body;
        ...
        console.log('🔍 DEBUG - Campo servicio:', {
      servicio: datosRemito.servicio,
      tipoServicio: typeof datosRemito.servicio,
      esArray: Array.isArray(datosRemito.servicio),
      datosCompletos: JSON.stringify(datosRemito, null, 2)
    });
    ...
    if (
      !datosRemito.productos ||
      !Array.isArray(datosRemito.productos) ||
      datosRemito.productos.length === 0
    ) {
      return res.status(400).json({
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=remito_${datosRemito.numeroRemito.replace(
        /[^a-zA-Z0-9]/g,
        "_"
      )}.pdf`
    );
